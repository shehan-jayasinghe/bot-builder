...
| **Chat assembly** | `FinalPromptBuilder` adds personality, tone, and `GuardrailRunner.build_instructions()` at runtime. `GuardrailRunner.check()` is pass-through by default; optional NeMo intent gate when `NEMO_GUARDRAILS_ENABLED=true`; optional NeMo output gate after graph turn — [migration-nemo-guardrails.md](./migration-nemo-guardrails.md). |
