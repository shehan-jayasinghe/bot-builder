...
**Agentic migration (chat API):** Phase A **Done**. Phase B **no change** (workflows only). Phase C **planned** — remove sub-agent auto-RAG prefetch; `search_knowledge` on `SubAgentRunner` turn. Phase D — sticky sub-agent. See [../agentic/updets/api-migration-agentic-phase-c.md](../agentic/updets/api-migration-agentic-phase-c.md).
