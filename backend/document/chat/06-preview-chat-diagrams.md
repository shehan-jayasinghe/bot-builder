...
**Agentic migration:** Phase A **Done** — no route/schema change. Phase B **Done** — same URLs; runtime no longer auto-starts workflows on first message. Phase C **planned** — agentic `search_knowledge` RAG (no REST change). See [../agentic/updets/api-migration-agentic-phase-c.md](../agentic/updets/api-migration-agentic-phase-c.md).
